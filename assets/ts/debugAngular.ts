...
export function debugAngular() {
  (function () {
    ...
    function clog(message1: string, message2?: any, message3?: any) {
      if (debug) {
    ...
    $rootScope.kubut.$stateChangeStart = $rootScope.$on(
      "$stateChangeStart",
    ...
    $rootScope.kubut.$stateChangeError = $rootScope.$on(
      "$stateChangeError",
      function (event, toState, toParams, fromState, fromParams) {
        clog(
    ...
    $rootScope.kubut.$stateChangeSuccess = $rootScope.$on(
      "$stateChangeSuccess",
      function (event, toState, toParams, fromState, fromParams) {
        clog(
    ...
    $rootScope.kubut.$viewContentLoaded = $rootScope.$on(
      "$viewContentLoaded",
      function (event) {
    ...
    $rootScope.kubut.$stateNotFound = $rootScope.$on(
      "$stateNotFound",
