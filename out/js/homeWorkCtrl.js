...
app_module.controller('homeWorkCtrl', [
    '$scope', 'homeworkHttp', '$location', '$mdDialog', '$filter', '$compile', '$sce', '$templateRequest',
    '$mdToast', '$element', 'baseHttp', 'localStorageService', 'DIRECTORY_TYPE','$rootScope', 'reportHttp', homeWorkCtrl]);

function homeWorkCtrl ($scope, homeworkHttp, $location, $mdDialog, $filter, $compile,
                       $sce, $templateRequest, $mdToast, $element, baseHttp, localStorageService, DIRECTORY_TYPE,
                       $rootScope, reportHttp) {
    ...
    $scope.actievTabType = { // номер активного таба 1 - домашние работы, 2 - лабораторные, 3 - пересдача
        new_hw: 1,
        new_lab: 2,
        request_stud: 3
    };
    ...
    $scope.hwStatuses = ['hw_not-loaded', 'hw_checked', 'hw_new', 'hw_retake', 'hw_overdue'];

    $scope.showPrerenderedDialog = function() {
        if (!!$scope.new_hw.length || !!$scope.new_lab.length || !!$scope.request_stud.length) {
            $mdDialog.show({
    ...
    $scope.getHomeworks = function()
    {
        homeworkHttp.getHomeworks({
            ...
            $scope.table_header = ($scope.table_header_full.length > $scope.limit) ? $scope.table_header_full.slice($scope.offset, ($scope.offset) +  $scope.limit) : $scope.table_header_full;
            $scope.startPosition =  $scope.table_header_full.length - $scope.offset;
            $scope.endPosition   =  ($scope.table_header_full.length > ($scope.offset + $scope.limit)) ? ($scope.table_header_full.length - $scope.offset - $scope.limit + 1) : 1;
                ...
                if(r.data) {
                    angular.forEach(r.data, function (value, key) {
                        if(angular.isDefined( $scope.table_body[value['stud']])) {
                            $scope.table_body[value['stud']][value['id']].id_domzadstud = value['id_domzadstud'];
                        }
                    });
                }
            }
    ...
    $scope.changeOffset = function(k){
        ...
        if($scope.table_header_full.length >= $scope.offset) {
            $scope.table_header = $scope.table_header_full.slice($scope.offset, ($scope.offset) + $scope.limit);
            $scope.startPosition =  $scope.table_header_full.length - $scope.offset;
            $scope.endPosition = ($scope.table_header_full.length > ($scope.offset + $scope.limit)) ?  ($scope.table_header_full.length - $scope.offset - $scope.limit+1) : 1;
        }else{
            $scope.offset = $scope.offset - k*$scope.limit;
    ...
    $scope.deleteHomework = function(id, comment, commentAttach){
        if(angular.isDefined(commentAttach)){ //если есть прикрепленный файл к комментарию
            baseHttp.getFileUploadToken().success(function(credentials) {
                if (angular.isDefined(credentials.token)) {
                    baseHttp.uploadFile(credentials, commentAttach, DIRECTORY_TYPE.HOMEWORK_COMMENT_ATTACHMENT).success(function (rU) {
                        if (angular.isDefined(rU[0].link)) {
                            $scope.baseDeleteHw({id_domzad : id, comment: comment, comment_attach : rU[0].link});
                        }
                    });
                }
            });
    ...
    $scope.addIsAutotest = function(data) {
        if (!data) return;
        data.forEach(function(item) {
            item.is_autotest = item.auto_test_response ? ($scope.isValidJson(item.auto_test_response) ? 2 : 1) : undefined;
            if (item.is_autotest === 2) {
                try {
                    var autoTestResponse = JSON.parse(item.auto_test_response);
                    item.coment = autoTestResponse.response;
                    item.mark = autoTestResponse.mark;
                } catch (e) {
                    // оставляем auto_test_response без изменений
                }
            }
        });
    };
     ...
     * @param isShowPrerender
     */
    $scope.getModalData = function(activeTab, isShowPrerender){
        homeworkHttp.getNewHomework().success(function(r){

            // Добавляем is_autotest
            $scope.addIsAutotest(r.homework);
            $scope.addIsAutotest(r.lab_work);
            $scope.addIsAutotest(r.requests);
            ...
            $scope.studWorkData = r.homework.length && r.homework || r.lab_work.length && r.lab_work || r.requests.length && r.requests || null;
            ...
            if(r.new_true && myWidth > 992){
                ...
                if (activeTab) {
                    $scope.studWorkData = activeTab == $scope.actievTabType.new_hw ? $scope.new_hw
                        : (activeTab == $scope.actievTabType.new_lab ? $scope.new_lab : $scope.request_stud);
                    $scope.activeHome = activeTab;
                }
                // если вкладка была задана ранее и в ней остались не проверенные дз - отображаем ее
                else if ($scope.activeHome && arrRequest[$scope.activeHome].length > 0) {
                ...
                // в остальных случаях отображаем первую вкладку, в которой есть не проверенные дз
                    ...
                    $scope.activeHome = !!$scope.new_hw.length ? 1 : ( !!$scope.new_lab.length ? 2 : (!!$scope.request_stud.length ? 3 : null))
                }
            }
        });
    };
    ...
    $scope.showConfirmDeleteDzStud = function(id, inDialog) {
        var confirm = $mdDialog.confirm({
            ...
            $mdDialog.show(confirm).then(function () {
                $scope.deleteHomework(id, $rootScope.deletingDz.comment, $rootScope.deletingDz.file_attach);
    ...
    function ConfirmDzDialogController($scope, $mdDialog, $rootScope) {
        ...
        $scope.setParams= function(params){
            if(angular.isDefined(params)){
                if(angular.isDefined(params.comment) || angular.isDefined(params.file_attach)){
                ...
                $mdToast.show({
                    hideDelay   : 4000,
                    position    : 'top right',
                    template: '<md-toast class="md-toast red">' + $filter('translate')('write_reason_delete_dz') + '</md-toast>'
                });
            }
        };
    }
    ...
    $scope.deleteHomeworkTeach = function(id){
        ...
        confirm.title($filter('translate')('confirm_delete_dz'))
    ...
    $scope.saveComment = function(id, comment, idStud, attach, works){
        if(angular.isDefined(attach)){
            baseHttp.getFileUploadToken().success(function(credentials) {
                if (angular.isDefined(credentials.token)) {
                    baseHttp.uploadFile(credentials, attach, DIRECTORY_TYPE.HOMEWORK_COMMENT_ATTACHMENT).success(function (rU) {
                        if (angular.isDefined(rU[0].link)) {
    ...
    $scope.saveCommentBase = function(id, comment, idStud, filename, works){
        homeworkHttp.saveComment({id : id, comment : comment, id_stud : idStud, attach : filename}).success(function(r){
            if(r.error) {
                $mdToast.show({
                    hideDelay   : 4000,
                    position    : 'top right',
    ...
    $scope.showComment = function(commentObj){
        var marksBlock = $sce.getTrustedResourceUrl('views/templates/homeworkComment.html');
        $scope.ActiveComment = commentObj;
        $templateRequest(marksBlock).then(function(template) {
            $compile($('#comment_' + commentObj.id_domzadstud).html(template).contents())($scope);
        }, function() {
    ...
    $scope.showCommentMenu = function(commentObj){
        var marksBlock = $sce.getTrustedResourceUrl('views/templates/homeworkCommon.html');
        $scope.ActiveComment = commentObj;
        $templateRequest(marksBlock).then(function(template) {
    ...
    $scope.deleteAttachComment = function(idDz, idStud, obj){
        homeworkHttp.deleteAttachComment({id : idDz, id_stud : idStud}).success(function(r){
            if(r.error) {
                $mdToast.show({
                    hideDelay   : 4000,
                    position    : 'top right',
    ...
    $scope.validateCommentLength = function(commentObj, type = null){
        const textLimit = (type && type == 2) ? 65535 : 1000;
        if(angular.isDefined(commentObj.coment)){
            commentObj.coment = commentObj.coment.length > textLimit ? commentObj.coment.slice(0,textLimit) : commentObj.coment;
        }
    ...
    $scope.showTheme = function (e) {
        if (e.target.matches('.hw-md_theme__more') || e.target.matches('.hw-md_single_theme__more')) {
    ...
    $scope.showHomeworkStud = function(studHw, isNewModal) {
        // Создаем глубокую копию studHw
        $scope.ActiveStudHw = angular.copy(studHw);
        $scope.ActiveStudHwNewValue = angular.copy(studHw);
        $scope.ActiveStudHw.is_autotest = $scope.table_header_full.find(item => item.id_domzad === studHw.id_domzad)?.is_autotest || null;

        // Проверяем наличие данных в поле auto_test_response
        if ($scope.ActiveStudHw.auto_test_response && $scope.ActiveStudHw.is_autotest == 2) {
            try {
                var autoTestResponse = JSON.parse($scope.ActiveStudHw.auto_test_response);
                $scope.ActiveStudHwNewValue.coment = autoTestResponse.response;
                $scope.ActiveStudHwNewValue.mark = autoTestResponse.mark;
            } catch (e) {
                $scope.ActiveStudHw.auto_test_response = null;
            }
        }
    ...
    $scope.saveCommentFile = function(id, idStud, attach){
        return new Promise((resolve, reject) => {
            if(attach){
                baseHttp.getFileUploadToken().success(function(credentials) {
                    if (angular.isDefined(credentials.token)) {
                        baseHttp.uploadFile(credentials, attach, DIRECTORY_TYPE.HOMEWORK_COMMENT_ATTACHMENT).success(res => {
                            resolve(res);
                        }).error(function (eR){
                            // Если мы не смогли отправить файл на сервер выбрасываем ошибку приложения
                            $mdToast.show({
    ...
    /**
     * Сохранение ответа на ДЗ - ссылка на файл от преподавателя, оценка, комментарий преподавателя
     * @param studDz
     * @param isNewHw - фаг, сохраняется ли дз из модалки непроверенных домашних(а так же пересдача и лаб. работы)
    ...
    $scope.saveStudHw = function(studDz, isNewHw) {
        ...
        $scope.saveCommentFile(studDz.id_domzad, id_stud =(studDz.stud_stud || studDz.id_stud), $scope.teachFile).then(res => {
            ...
            // если есть ссылка на файл от преподавателя, то передаем ее
            if (res && res[0].link) {
            ...
            homeworkHttp.saveHomework({HomeworkForm: studDz}).success(function(r) {
                if(r.error) {
                    $mdToast.show({
                        hideDelay   : 4000,
                        position    : 'top right',
                    ...
                    if ($scope.ActiveStudHwNewValue) {
                        $scope.ActiveStudHwNewValue.comment_attach_file = r.responseComment.attach_name;
                    }
    ...
    /**
     * Если строка имеет спецсимволы html, то этот вызова дает возможность вывести корректный вид строки
     * @param str
     * @returns {*}
     */
