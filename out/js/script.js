var modules = ['ui.router', 'ngMaterial', 'ngMessages', 'LocalStorageModule', 'angular.chosen', 'pascalprecht.translate', 'angular-toArrayFilter', 'tableSort'];

if(YII_ENV ==='prod') {
    // Raven
    //     .config('https://1c9e6cb865934a15aacde14b34053bd9@sentry.itstep.org/23')
    ...
    app.config(function($stateProvider, $urlRouterProvider, $mdThemingProvider) {
        ...
        $stateProvider
            ...
            .state('bind.teach_materials', {
                ...
                templateUrl : '/index/get-view?template=bind/bind_teach_materials_for_teach',
                controller  : 'bindTeachMaterialsCtrl'
            })
            .state('bind.forms', {
                url : '/forms',
    ...
    app.factory('httpInterceptor', ['$q','$location',function ($q, $location) {
        return {
            'request': function(config) {
                if(config.url.indexOf('http') !== 0){ //не передаем заголовки на внешние подключения типа файлменеджера
                    config.headers['Id-Local-Hash'] = sessionStorage.getItem('IdLocalHash'); //заголовки для идентификатора http запросов
                    // todo если нужны будут headers для языка как в админлогбуке. Нужно чтобы кука lang с бэка передавалась не зашифрованная и доступная к просмотру
                    // let matches = document.cookie.match(new RegExp(
                    //     "(?:^|; )" + 'lang'.replace(/([\.$?*|{}\(\)\[\]\\\/\+^])/g, '\\$1') + "=([^;]*)"
                    // ));
                    // console.log('matches2', matches);
                    // let lang =  matches ? decodeURIComponent(matches[1]) : 'en';
            ...
            'responseError': function(rejection) {
                ...
                }else if (rejection.status === 409) { //Ошибка если пользователь активной страницы отличается от пользователя авторизации
...
app.config(['$translateProvider', function ($translateProvider) {
    $translateProvider.useUrlLoader('/translate', {async: false, queryParameter: 'lang_id'});
    ...
    app.run(function($translate, $mdDateLocale, $filter){
        $translate.onReady(function() {
            ...
            for(var i = 0; i < 12; i++){
                ...
                $mdDateLocale.shortMonths.push($filter('translate')('month_short_name_' + i));
            }
